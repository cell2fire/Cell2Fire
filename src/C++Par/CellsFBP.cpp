...
std::vector<int> CellsFBP::manageFire(int period, std::unordered_set<int> & AvailSet,      
	...
	// Local copy to make the function thread safe 
	inputs * df_ptr2 = df_ptr;
	df_ptr2->waz = wdf_ptr->waz;
	df_ptr2->ws = wdf_ptr->ws;
	df_ptr2->ffmc = wdf_ptr->ffmc;
	df_ptr2->bui = wdf_ptr->bui;	
	...
	calculate(df_ptr2, coef, &mainstruct, &sndstruct, &headstruct, &flankstruct, &backstruct);
    ...
    if (HROS > args->ROSThreshold && headstruct.fi > args->HFIThreshold) {
       ...
       // vector<double> toPop = vector<double>();
        ...
        for (auto&  _angle : this->ROSAngleDir) {
			...
			// TODO Check FSCell (probably make a copy to avoid thread "unsafe" process in the parallel section)
