...
std::vector<std::unordered_map<int, vector<int>>> Cell2Fire::SendMessages(){
	...
	std::vector<std::vector<int>> burnedOutListAux = std::vector<std::vector<int>>(this->args.nthreads);
	...
	std::vector<std::unordered_map<int, vector<int>>> sendMessageList(this->args.nthreads);
	...
	for (int h=0; h < burningCells2.size(); h++) {
		...
		*/
		if (it->second.ROSAngleDir.size() > 0) {
			//#pragma omp critical // In case the function is not thread safe 
			//{
