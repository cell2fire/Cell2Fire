...
// Send messages 
std::vector<std::unordered_map<int, vector<int>>> Cell2Fire::SendMessages(){
	// Iterator
	std::unordered_map<int, CellsFBP>::iterator it;
	
	// Clean list 
	std::vector<std::vector<int>> burnedOutListAux = std::vector<std::vector<int>>(this->args.nthreads * PADDING);
	...
	std::vector<std::unordered_map<int, vector<int>>> sendMessageList(this->args.nthreads * PADDING);
	...
	// Aux vector with burning cells for parallel zone
	std::vector<int> burningCells2;
	for (auto &i : this->burningCells) {
		burningCells2.push_back(i);
    }
	
	// Parallel Zone
	#pragma omp parallel for num_threads(this->args.nthreads)
	for (int h=0; h < burningCells2.size(); h++) {
		int tid = omp_get_thread_num() * PADDING;
		int cell = burningCells2[h];
		std::vector<int> aux_list;
		
		// Get object from unordered map
		auto it = this->Cells_Obj.find(cell);
		...
		*/
		...
		//std::cout << "Pre aux list mayor 0" << std::endl;
		...
		if (aux_list.size() > 0){
			...
			if (aux_list[0] != -100) {
				...
				sendMessageList[tid][it->second.realId] = aux_list; 
				
				if (this->args.verbose){
					std::cout << "Message list content" << std::endl;
					for (auto & msg : sendMessageList[tid][it->second.realId]){
		...
		//std::cout << "Pre aux list igual 0" << std::endl;
		// Burnt out inactive burning cells
		if (aux_list.size() == 0) {
			burnedOutListAux[tid].push_back(it->second.realId);
			if (this->args.verbose){
	...
	for(auto & v : burnedOutListAux){
		for (auto & bc : v) {
			this->burnedOutList.push_back(bc);
...
// Get messages
void Cell2Fire::GetMessages(std::vector<std::unordered_map<int, vector<int>>> sendMessageList){
	// Iterator 
	std::unordered_map<int, CellsFBP>::iterator it;
...
void Cell2Fire::Results(){
	...
	******************************************************************************/
	...
	if(this->args.OutMessages){
		...
		CSVPloter.printCSVDouble(this->burntCells.size() - 1, 4, this->FSCell);
	}
...
void Cell2Fire::Step(std::default_random_engine generator){
	...
	if (this->fire_period[this->year - 1] > 0 && !this->done){
		...
		std::vector<std::unordered_map<int, vector<int>>> SendMessageList = this->SendMessages();
	...
	if ((this->availCells.size() == 0 || this->burningCells.size() == 0) && !this->done && !auxC){
		// Done
		this->done = true;
...
int main(int argc, char * argv[]){
	...
	int rnumber = udistribution(generator);
	double rnumber2 = ndistribution(generator);
